class EventBus<T extends Record<string, unknown>> {
  ...
  public on<K extends keyof T>(
    ...
    console.log(
      `Channel ${callback.name} turned on with ${eventName.toString()}`,
    );
  ...
  public off<K extends keyof T>(
    ...
    console.log(
      `Channel ${callback.name} turned off having ${eventName.toString()}`,
    );
    this.identifyFunction[eventName] = this.identifyFunction[eventName].filter(
      (data) => data !== callback,
    );
  }
...
function updateInventory(item: {
  ...
  console.log(
    `[Inventory] Reducing stock for item ${item.productId} by ${item.quantity}`,
  );
}
...
function sendAnalytics(item: {
  ...
  console.log(
    `[Analytics] Tracked addition of ${item.productId} ($${item.price})`,
  );
}
function successCheckout(item: { orderId: string; totalAmount: number }) {
  console.log(
    `[Checkout] Successful checkout of orderId: ${item.orderId} Total Amount:($${item.totalAmount})`,
  );
}
